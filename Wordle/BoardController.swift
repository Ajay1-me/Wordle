...
class BoardController: NSObject,
  ...
  func resetBoardWithCurrentSettings() {
    // START YOUR CODE HERE
      var currentSetting: [String: Any] = [:]
      //implemented the below script here so that the "rawTheme" doesn't remain outside the scope
      let rawTheme = SettingsManager.shared.settingsDictionary[kWordThemeKey] as! String
      currentSetting[kNumLettersKey] = numItemsPerRow
      currentSetting[kNumGuessesKey] = numRows
      currentSetting[kWordThemeKey] = WordTheme(rawValue: rawTheme)
      currentSetting[kIsAlienWordleKey] = isAlienWordle
      
      resetBoard(with: currentSetting)
    // END YOUR CODE HERE
  }
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      //if let statementn allow to unwrap safely
